...
class Recpt :
   ...
   #===============
   def __init__(self,outp,h,dbfn = "/home/public/web/invapp/inv.db") :
      self.outp = outp
      self.h = h
      self.dbfn = dbfn
      ...
      for nm in self.map :
         for i in count() :
            ...
            elif sum( len(v) for v in rr.values() ) == 0 : break
   ...
   def _getloc(self,key) :
      ...
      row = self.cur.fetchone()
      if not row : six.print_( "No loc %s" % str(key) )
      return row[0]
   #===============
...
class Tag :
   ...
   def __str__(self) :
      return "<%s%s>%s%s</%s>\n" % (self.tag,
        "".join([ (""" %s="%s" """% (k,v)) 
                 for k,v in self.attr.items() if k if v]),
        self.t or '',
...
def encUID(uid) :
   ...
   return base64.b64encode( ("%s%s%s*%s" % 
                             (str(pad),pv(pad*5),uid,pv(pad))
                            ).encode('utf8') ).decode('utf8')
def decUID(uid) :
   import base64
   if not uid : return uid
   v = base64.b64decode(uid.encode('utf8')).decode('utf8')
   ps = int(v[0])
   if v[-ps-1] != '*' :
...
def hashpwd(pwd) :
   md5 = hashlib.md5()
   md5.update( ("%s::pilsung" % pwd).encode('utf8') )
...
def main() :
   ...
   result = sio()
   handler = Recpt(result,form,"/home/public/web/invapp/inv.db")
   try :
...
try :
   ...
   six.print_( "Content-Type: text/html" )   # HTML is following
   six.print_('')                            # blank line, end of headers

   six.print_( "<html><head/><body><TITLE>CGI script output</TITLE>" )
   six.print_( "<H1>Error</H1><br/>" )
   six.print_( "%s</body></html>" % traceback.print_exc(file = sys.stdout) )
