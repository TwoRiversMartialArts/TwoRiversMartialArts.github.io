...
$.googleMaps = {
    ...
    mapMarkers: function(center, markers) {
        ...
        for (i = 0; i < markers.length; i++) {
            ...
            //ADDED 4/28/2010 by ksg
            var currMarkerLat = markers[i].latitude;
            var currMarkerLng = markers[i].longitude;
            var currMarkerInfo = markers[i].info.layer;
            GEvent.addListener($.googleMaps.marker[i], 'click', function() { try { recordClickEvent(currMarkerLat, currMarkerLng, currMarkerInfo); } catch (err) { } });
